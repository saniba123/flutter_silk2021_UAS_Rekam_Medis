...
class dbDetailResep extends StatefulWidget {
  List list;
  int index;
  dbDetailResep({Key key, this.title, this.list, this.index}) : super(key: key);

  final String title;

  @override
  _dbDetailResepState createState() => _dbDetailResepState();
}

class _dbDetailResepState extends State<dbDetailResep> {
  final _formKey = GlobalKey<FormState>();

  Future<List> getData() async{
    final response= await http.get("http://10.0.2.2/silk2021/flutter_silk/lib/Farmasi/crud/getdetail.php");
    ...
    return new ListView.builder(
        ...
        itemBuilder: (context, i) {
          return new Container(
            child: new Card(
              child: new ListTile(
                title: new Text("ID Detail Resep : ${list[i]['id_detil_resep'] + " || ID Resep : ${list[i]['id_resep']}"}"),
                leading: new Icon(Icons.widgets),
                subtitle: new Text("Kode Obat : ${list[i]['kode_obat'] + " || Harga : ${list[i]['harga'] + " || Jumlah Beli : ${list[i]['jumlah_beli']}"}"}"),
                onLongPress: (){
                  showDialog(
                      context: context,
